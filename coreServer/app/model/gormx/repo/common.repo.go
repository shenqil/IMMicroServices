...
func WrapPageQuery(ctx context.Context, db *gorm.DB, pp *pb.PaginationParam, out interface{}) (*pb.PaginationResult, error) {
	if pp.OnlyCount {
		var count int64
		...
		return &pb.PaginationResult{Total: count}, nil
...
// FindPage 查询分页数据
func FindPage(ctx context.Context, db *gorm.DB, pp *pb.PaginationParam, out interface{}) (int64, error) {
	var count int64
	err := db.Count(&count).Error
	if err != nil {
		return 0, err
...
// ParseOrder 解析排序字段
func ParseOrder(items []*pb.OrderField, handle ...OrderFieldFunc) string {
	...
	for i, item := range items {
		...
		direction := "ASC"
		if item.Direction == pb.OrderDirection_OrderByDESC {
			direction = "DESC"
		}
